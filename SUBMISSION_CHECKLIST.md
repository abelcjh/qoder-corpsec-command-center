...
- [x] Live Cloudflare Worker home route returned HTTP 200 app HTML in 363ms during the Aug 6 14:07 GMT+8 Ralph smoke check.
- [x] `POST /api/ai-brief` returned HTTP 200 fixture JSON in 11.4s in the same smoke check, proving the server-side Agnes bridge is reachable without exposing the key in the browser; no secret-name or API-key-shaped values were exposed.
