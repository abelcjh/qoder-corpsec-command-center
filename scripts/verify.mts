...
assert(submissionPack.includes('Aug 6 15:40 GMT+8'), 'Submission Pack includes latest Aug 6 15:40 live smoke receipt');
...
const aiWorkbench = readFileSync(resolve(root, 'src/components/screens/AIWorkbenchScreen.tsx'), 'utf-8');
assert(aiWorkbench.includes('AI approval receipt'), 'AI Workbench exposes judge-visible AI approval receipt');
assert(aiWorkbench.includes('READY_FOR_REVIEW'), 'AI Workbench shows deterministic ready-for-review gate verdict');
assert(aiWorkbench.includes('sending stays blocked in demo mode'), 'AI Workbench keeps no-send gate visible beside AI output');
