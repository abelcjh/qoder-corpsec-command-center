...
| Verification script with 165 assertions | `scripts/verify.mts` |
...
| AgentTrust / Agent Receipts approval-expiry scan (`https://github.com/Brilynd/agentTrust`, `https://agentreceipts.ai/`) | Agent execution layers increasingly prove step-up approval, human authorization, action identity, and tamper-evident audit trails; unresolved high-risk requests should expire or remain blocked instead of turning into hidden standing consent. | Add the Submission Pack approval timeout receipt: pending approval is not allowed to continue silently, the bounded review window expires into a retained no-send proof row, and real sends/forms/legal changes stay manual. |
...
- Live Worker smoke receipt from Aug 6 15:40 GMT+8 Ralph loop: `/` returned HTTP 200 app HTML in 487ms and `POST /api/ai-brief` returned HTTP 200 fixture JSON in 9.81s using a browser-like smoke request; no secret-name or API-key-shaped values were exposed. Use this as the demo reliability proof before recording; if the network stalls, fall back to the build+verify terminal receipt.
...
| Approval timeout receipt | Pending is not allowed → bounded review window → no silent continuation → expired no-send receipt proves unattended high-risk actions do not become hidden consent. | Submission Pack approval timeout panel; verifier assertions; final video fallback if judges ask what happens when an approver disappears |
...
| Evidence | Location |
|----------|----------|
| Aug 6 15:40 GMT+8 remote smoke check returned HTTP 200 app HTML for `/` on the Cloudflare Worker | terminal smoke receipt from Ralph loop; `src/components/screens/SubmissionPackScreen.tsx` |
| Same smoke check returned HTTP 200 JSON from `POST /api/ai-brief` with fixture-only request data and browser-like headers in 9.81s, while no secret-name or API-key-shaped values were exposed | terminal smoke receipt from Ralph loop; `src/components/screens/SubmissionPackScreen.tsx` |
